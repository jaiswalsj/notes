...
class NotesDatabase{
  ...
  Future<Note?> InsertEntry(Note note) async{
    ...
    //   final db = await instance.database;
    //   await db!.insert("Notes", { "pin" : 0, "title" : "THIS IS MY TITLE WRITTEN BY SJ", "content" : "THIS IS MY NOTE CONTENT", "createdTime": "26 Jan 2018"});
    //   return true;
    // }
  ...
  Future<List<Note>> readAllArchNotes()async{
    final db = await instance.database;
    final orderBy = '${NotesImpNames.createdTime} ASC';
    final query_result = await db!.query(NotesImpNames.TableName,orderBy: orderBy,where:'${NotesImpNames.isArchieve}=1',);
  ...
  Future<Note?> readOneNote(int id)async{
    ...
    final db = await instance.database;
    return await db!.update(NotesImpNames.TableName, {NotesImpNames.isArchieve:!note!.isArchieve?1:0},where: '${NotesImpNames.id}=?',whereArgs: [note.id]);
    }

    // Future updateNote(int id)async {
    // final db = await instance.database;
    // return await db!.update('Notes', {"title": "THIS IS A UPDATED TITLE"},where: 'id = ?',whereArgs: [id]);
    // }
